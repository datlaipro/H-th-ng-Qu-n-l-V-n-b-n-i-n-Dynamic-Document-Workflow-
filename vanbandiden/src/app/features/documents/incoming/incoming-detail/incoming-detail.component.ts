...
const MOCK_DATA: DocumentItem[] = [
  { id: 1, loai: 'Đến',  soHieu: 'VB-001/2025', ngayBanHanh: '2025-09-10', nguoiKhoiTao: 'Nguyễn An', trangThai: 'Chờ xử lý' },
  { id: 2, loai: 'Đến', soHieu: 'CV-778/UBND', ngayNhan: '2025-09-12', nguoiKhoiTao: 'Trần Bình', trangThai: 'Đang xử lý' },
  { id: 3, loai: 'Đến',  soHieu: 'VB-013/2025', ngayBanHanh: '2025-09-18', nguoiKhoiTao: 'Lê Chi',   trangThai: 'Hoàn thành' },
];
...
@Component({
  ...
  styleUrls: ['./incoming-detail.component.css']
})
export class IncomingDetail{
  ...
  constructor() {
    this.dataSource.filterPredicate = (row, filterStr) => {
      ...
      const haystack = [
        row.loai, row.soHieu, row.ngayBanHanh ?? '', row.ngayNhan ?? '', row.nguoiKhoiTao, row.trangThai
      ].join(' ').toLowerCase();
